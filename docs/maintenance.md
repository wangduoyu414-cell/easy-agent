...
- ChatGPT Windows：主路径固定下载 OpenAI 官方文档列出的微软轻量安装器 `9PLM9XGG6VKS`。刷新时对固定架构的 `ChatGPT-x64.msix` / `ChatGPT-arm64.msix` 执行白名单约束的 HEAD 请求，只接受与本地信任配置一致的 `x-ms-meta-package_identity`、`x-ms-meta-architecture`、四段 `x-ms-meta-package_version` 和安全范围内 `Content-Length`；该版本只用于状态比较和目标复检，完整安装包仍必须通过内部签名与清单验证。每次操作重新下载微软安装器，验证 Microsoft Authenticode、`MSStoreTag001` 签名配置中的 Product ID、PFN、`WindowsUpdate`、`isHarbor=true` 与 `autoUpdate=false` 后启动；不再探测、修复或升级 WinGet/App Installer。
