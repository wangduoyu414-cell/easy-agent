...
- HTTP 客户端启用系统代理发现，Finder 启动的桌面应用无需依赖 shell 代理环境变量；HTTPS、逐跳 host/path 和重定向上限仍不放宽。
...
- ChatGPT macOS 固定已签名官方应用中的 `SUPublicEDKey`，appcast `edSignature` 不再只做 Base64 语法检查；完整 ZIP 在下载后和执行前都进行 Sparkle Ed25519 验签。
...
- Claude stable redirect 仍返回带 `cf-mitigated: challenge` 的 HTTP 403，因此 Claude macOS 条目继续失败关闭，没有切换浏览器 cookie、旧地址或第三方镜像。
- 2026-08-08 完整包与生产 verifier proof 后，WorkBuddy、CC Switch 和 ChatGPT 的 macOS x64/ARM64 条目已启用。Claude 与 Hermes 继续按各自原因失败关闭，UI 会直接显示完整原因而不是统一的“官方版本信息暂不可用”。
...
- 2026-08-08 当前开发检查已通过：格式检查、68 项自动测试（另有 4 项环境型 proof 默认忽略）、Clippy `-D warnings`、Apple Silicon 全部 target 编译检查；两项官方在线元数据/启用计划 smoke test 也已显式通过且没有下载完整安装包。
...
- macOS：WorkBuddy、CC Switch 与 ChatGPT 的双架构完整包、Apple 身份和生产 verifier proof 已闭合并启用；CC Switch 与 ChatGPT 另有临时首次安装/更新激活 proof，详见 `evidence/macos-functional-parity-audit-2026-08-08.md`。WorkBuddy 厂商摘要错误会持续记录和监控，但不再阻断固定 Apple 身份的包；Claude stable redirect 与 Hermes bootstrap 最终状态仍待厂商侧条件闭合。正式发布前仍建议在 Intel/Apple Silicon 可回滚干净环境补真实 UI、断网/磁盘、权限和运行中应用矩阵。
...
因此当前状态是：`easy agent Windows x64 implementation complete; macOS direct-app parity implemented, with WorkBuddy, CC Switch and ChatGPT enabled on Intel and Apple Silicon; Claude and Hermes remain product-specifically fail-closed; release signing and clean-machine release validation pending`。当前 EXE 与 Mac DMG 都仍是开发验证产物；尚未生成 Apple Developer ID 签名并公证的正式 Mac DMG。
